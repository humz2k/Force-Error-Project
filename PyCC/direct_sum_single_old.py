...
def evaluate(particles,velocities,masses,eval_pos = None, steps = 0, eps = 0, G = constants.G,dt = 1000,eval_only=False):
    ...
    if eval_only:
        phi,acc,stats = phi_acc(particles.astype("f4"),masses.astype("f4"),eval_pos.astype("f4"),eps,G)
        acc = pd.DataFrame(acc,columns=["ax","ay","az"])
        phi = pd.DataFrame(np.reshape(phi,(1,len(phi))).T,columns=["phi"])
        df = pd.concat((acc,phi),axis=1)
        return df,stats
    ...
    if type(eval_pos) != type(None):
        ...
        temp_phi,temp_acc,eval_stats = phi_acc(pos,masses,evals,eps,G)
        eval_phi[0] = temp_phi
        eval_acc[0] = temp_acc
    else:
